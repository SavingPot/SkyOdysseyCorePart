...
public static class BackgroundRecompiler
{
    ...
    private static bool IsAssemblyLocked
    {
        get
        {
            if (CanReloadAssembliesMethod == null)
            {
                ...
                if (CanReloadAssembliesMethod == null)
                {
                    Debug.LogError("无法加载 CanReloadAssemblies 方法. 可能被重命名或删除了");
                }
            }
    ...
    public static void SetActive(bool newState)
    {
        ...
        if (newState)
        {
            ...
            // create watcher in a separate thread to prevent unity from freezing and increasing recompile times
    ...
    // custom logging
    private const string LOG_PREFIX = "[ 背景编译器 ] - ";
    private const string LOG_POSTFIX = "\n\n- 你可以在 \"Preferences/Plugins/Background Recompiler\". 中关闭日志\n";
